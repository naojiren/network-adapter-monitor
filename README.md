...
- 实时速度每 2 秒刷新；IP、网关、MAC 等详细信息每 20 秒刷新，并监听 Windows 网络变化事件。
...
- 托盘速度显示固定在任务栏隐藏图标箭头左侧，每 5 秒重新识别位置，随任务栏布局变化自动调整。
- 托盘速度显示会自动匹配任务栏背景色；浅色背景显示黑字，深色背景显示白字。
- 首次运行自动加入当前用户的开机启动项，无需管理员权限。
- 电脑开机自动启动时默认进入托盘状态；手动打开程序时正常显示主窗口。
- 电脑超过 5 分钟无鼠标/键盘操作时自动暂停刷新，降低资源占用；空闲时 CPU 占用可降到 0%，恢复操作后自动继续刷新。

## 资源占用优化

空闲超过 5 分钟后，程序会暂停刷新网卡状态，任务管理器中 CPU 占用可降到 0%。

![空闲时 CPU 占用为 0%](无操作cpu占用为0.png)
...
请在右侧或页面下方的 [Releases](https://github.com/naojiren/network-adapter-monitor/releases/latest) 下载编译好的压缩包。

1. 下载 `Network.Adapter.Monitor.zip`。
2. 解压到固定目录。
3. 运行解压后的程序。
